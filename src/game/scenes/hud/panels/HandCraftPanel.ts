...
export class HandCraftPanel extends HudPanel {
    ...
    protected create(): void {
        ...
        this.background = this.scene.rexUI.add.roundRectangle(0, 0, 10, 10, 7, 0x08110c, 0.88)
        ...
        this.title = this.scene.add.text(0, 0, 'HANDCRAFT', {
    ...
    update(ctx: HudContext): void {
        const availableRecipes = CRAFT_RECIPES.filter((recipe) => !recipe.station);
        const visible = ctx.handcraftOpen && !this.tiny && availableRecipes.length > 0;
        ...
        this.config.height = 22 + availableRecipes.length * (rowHeight + 4) + 24;
        ...
        for (const row of this.rows) {
            ...
            const available = !recipe.station;
